...
export default function EmployeeTicketsPage() {
  ...
  const fetchData = async () => {
    ...
    setTours(toursData.map(t => ({...t, date: new Date(t.date)})));
  ...
  useEffect(() => {
    ...
    const confirmedReservations = reservations.filter(
        (r: Reservation) => r.status === 'Confirmado' && r.sellerId === user.id
    );
    ...
    const generatedTickets = confirmedReservations.flatMap((res: Reservation): Ticket[] => {
        ...
        return res.passengerIds.map(passengerId => {
            ...
            const qrData = { tId: res.id, pId: passenger.id };
  ...
  }, [reservations, tours, passengers, user]);
  ...
  const toursWithTickets = useMemo(() => {
      ...
      return tours.filter(t => t.date && tripIdsWithTickets.has(t.id) && new Date(t.date) >= new Date());
  }, [allTickets, tours]);
 ...
 const handleDownload = async (ticket: Ticket) => {
    ...
    try {
        ...
        toast({
            ...
            description: "No se pudo generar el PDF. Puede que falte el logo de la empresa. Pídele al administrador que lo suba en Configuración.",
            variant: "destructive",
            duration: 7000
        });
    }
