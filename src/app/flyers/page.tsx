    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const [flyersData, toursData] = await Promise.all([
                getAllFromCollection<Flyer>('flyers'),
    ...
    return (
        ...
        <main className="flex-1">
            <Dialog open={isViewerOpen} onOpenChange={setIsViewerOpen}>
                <DialogContent className="max-w-4xl max-h-[90vh] bg-transparent border-none shadow-none">
                    <DialogHeader>
            ...
            {isLoading ? (
                ...
                <div className="space-y-16">
                    ...
                    {Object.values(flyersByTrip).map(({ tour, flyers }) => (
                         <div key={tour.id}>
                             <h2 className="text-2xl font-bold tracking-tighter sm:text-4xl font-headline mb-8">Próximo Viaje: {tour.destination}</h2>
                              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                {flyers.map(flyer => (
