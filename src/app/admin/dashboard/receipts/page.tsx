...
import { Receipt as ReceiptIcon, Printer, Pencil, Trash2, Text, Palette, FontSize, Upload, Save, ZoomIn, ZoomOut, Maximize, Bold, Italic, Type, CalendarPlus, Loader2 } from "lucide-react"
...
import { getAllFromCollection, getDocumentById, saveDocument, uploadFileAndGetURL } from "@/lib/firestore-services";
...
export default function ReceiptsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        if (templateData) {
            ...
            if (templateData.zoom) {
                setZoom(templateData.zoom);
            }
        }
    };
    fetchData();
  ...
  const activeTours = useMemo(() => {
    ...
    return tours.filter(t => t.date && new Date(t.date) >= new Date() && reservationTripIds.has(t.id));
  }, [tours, reservations]);
  ...
  const saveTemplate = async () => {
    ...
    try {
        ...
        if (page1ImageFile) {
            page1Url = await uploadFileAndGetURL(page1ImageFile, `receipts/template-bg1-${Date.now()}`);
        }
        if (page2ImageFile) {
            page2Url = await uploadFileAndGetURL(page2ImageFile, `receipts/template-bg2-${Date.now()}`);
        }
        ...
        // Update local state with the final URLs
        setTemplate(prev => ({
            ...prev,
        ...
        toast({ title: "Error al Guardar", description: "No se pudo guardar la plantilla. Verifica las imágenes.", variant: "destructive" });
    } finally {
  ...
  const handlePrint = async () => {
    ...
    try {
        ...
        toast({ title: 'Error', description: 'No se pudo generar el PDF para imprimir.', variant: 'destructive'});
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
       ...
       <Card>
        ...
        <CardContent className="flex flex-col sm:flex-row items-start gap-4">
            <div className="space-y-2 w-full sm:w-1/2">
                ...
                <Select onValueChange={(val) => { setSelectedTripId(val); setSelectedReservationId(""); }} value={selectedTripId || ''}>
                    ...
                    <SelectContent>{activeTours.map(tour => <SelectItem key={tour.id} value={tour.id}>{tour.destination}</SelectItem>)}</SelectContent>
                </Select>
            </div>
             <div className="space-y-2 w-full sm:w-1/2">
        ...
        {selectedReservation && template.fields.some(f => f.type === 'editableText') && (
            <Card>
                <CardHeader>
                    <CardTitle>Campos Editables</CardTitle>
                    <CardDescription>Completa el texto personalizado para este recibo.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    {template.fields.filter(f => f.type === 'editableText').map(field => (
                        <div key={field.id} className="space-y-2">
                            <Label htmlFor={`editable-${field.id}`}>{field.label}</Label>
                            <Input
        ...
        {selectedReservation && generatedReceiptData ? (
             <div className="w-full overflow-x-auto py-4">
                <div className="flex justify-center">
                    <div
                        style={{
                            transform: `scale(${template.zoom || 0.5})`,
                                ...
                                <div key={pageIndex} style={pageStyle} className="relative bg-white shadow-lg shrink-0">
                                    ...
                                    {template.fields.filter(f => f.page === pageIndex).map(field => {
                                        ...
                                        if (field.type === 'editableText') {
                                            ...
                                            content = generatedReceiptData[field.type as keyof typeof generatedReceiptData];
                                        }

                                        return (
                                            <div key={field.id} style={{ top: field.y, left: field.x, fontSize: `${field.size || 12}px`, color: field.color, fontFamily: field.font, fontWeight: field.isBold ? 'bold' : 'normal', fontStyle: field.isItalic ? 'italic' : 'normal' }} className={`absolute whitespace-nowrap`}>
                                                {content}
                                            </div>
                                        );
                                    })}
