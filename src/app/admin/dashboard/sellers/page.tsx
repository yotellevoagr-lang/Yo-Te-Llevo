  ...
  const handleCommissionRuleChange = (id: string, field: keyof CommissionRule, value: any) => {
    setCommissionSettings(prev => ({
      ...
      rules: prev.rules.map(rule => {
        if (rule.id === id) {
          if (field === 'from' || field === 'rate') {
             const numericValue = value === '' ? '' : Number(value);
             return { ...rule, [field]: numericValue };
          }
          if (field === 'to') {
            // Allow setting "infinite" or a number
            if (value === 'infinite') {
                return { ...rule, to: 'infinite' };
            }
            const numericValue = value === '' ? '' : Number(value);
            return { ...rule, to: numericValue };
          }
        }
        return rule;
      })
              ...
              {commissionSettings.rules.map((rule, index) => (
                <div key={rule.id} className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    <Input type="number" placeholder="Hasta" value={rule.to === 'infinite' ? '' : rule.to || ''} onChange={e => handleCommissionRuleChange(rule.id, 'to', e.target.value)} className="w-24"/>
                     {rule.to === 'infinite' && <InfinityIcon className="w-4 h-4 absolute top-1/2 right-2 -translate-y-1/2 text-muted-foreground"/>}
                  </div>
                  <Button variant="outline" size="icon" onClick={() => handleCommissionRuleChange(rule.id, 'to', rule.to === 'infinite' ? (rule.from || 0) + 1 : 'infinite')}><InfinityIcon className="w-4 h-4"/></Button>
