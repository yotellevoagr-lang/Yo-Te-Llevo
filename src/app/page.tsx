...
import type { Tour, GeneralSettings, Flyer } from "@/lib/types"
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function Home() {
 ...
 const carouselItems = useMemo(() => {
    return activeTours
      .map(tour => {
        if (!tour.backgroundImage) return null;
        return {
          id: tour.id,
          image: tour.backgroundImage,
        ...
        <section className="container py-16 md:py-24">
            <div className="grid items-center gap-12 md:grid-cols-2">
                ...
                <div className="relative w-full h-80 lg:h-96 animate-fade-in-up" style={{ animationDelay: "0.2s"}}>
                    {generalSettings?.aboutUsMedia ? (
                        generalSettings.aboutUsMedia.type === 'video' ? (
                            <video
                                src={generalSettings.aboutUsMedia.url}
                                autoPlay
                                loop
                                muted
                                playsInline
                            ...
                            <Image src={generalSettings.aboutUsMedia.url} alt="Sobre Nosotros" className="object-cover rounded-2xl shadow-2xl" layout="fill" data-ai-hint="happy travelers" />
                        )
                    ) : (
